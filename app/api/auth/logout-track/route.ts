...
export async function POST(request: NextRequest) {
  try {
    ...
    // Admins may end any session; service role bypasses RLS that restricts
    // updates to auth.uid() = user_id.
    if (!hasAdminClient()) {
      return NextResponse.json(
        { error: 'Admin database client not configured' },
        { status: 503 },
      );
    }
    ...
    if (endError) {
      console.error('logout-track admin end error:', endError.message);
      return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
    }
